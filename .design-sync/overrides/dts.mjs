// forked from design-sync lib/dts.mjs — CTA is a real component, not a constant
//
...
// Names the repo pinned in cfg.componentSrcMap are components by definition —
// an explicit pin is a stronger signal than any name-shape heuristic. Without
// this, `CTA` is read as a SCREAMING_CASE constant and silently dropped, and
// renaming the export to dodge that would make the emitted .d.ts contract
// describe an API the repo doesn't actually have.
const PINNED = (() => {
  try {
    const cfgPath = join(dirname(fileURLToPath(import.meta.url)), '..', 'config.json');
    return new Set(Object.entries(JSON.parse(readFileSync(cfgPath, 'utf8')).componentSrcMap ?? {})
      .filter(([, v]) => v !== null)
      .map(([k]) => k));
  } catch {
    return new Set();
...
export function findTypesRoot(pkgDir, pkgJson) {
  // This repo is a Next app, not a published package: there is no shipped .d.ts
  // tree, and the stock probe settles on the repo's own lib/ (which has none),
  // leaving every component with an empty `[key: string]: unknown` contract.
  // build-ds.mjs emits real declarations here via tsc --emitDeclarationOnly.
  const emitted = join(pkgDir, '.design-sync', '.cache', 'types');
...
export const isComponentName = (n) => PINNED.has(n) || (!n.endsWith('Props') && !/^[A-Z][A-Z0-9_]+$/.test(n)
